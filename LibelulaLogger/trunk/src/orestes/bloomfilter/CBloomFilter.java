...
public class CBloomFilter<T> extends BloomFilter<T> {
    ...
    /**
     * Constructs a new counting bloom filter by determining the optimal bloom
     * filter size <i>n</i> in bits and the number of hash functions <i>k</i>
     * based on the expected number <i>n</i> of elements in the bloom filter and
     * the tolerable false positive rate <i>p</i>.
     *
     * @param n Expected number of elements inserted in the bloom filter
     * @param p Tolerable false positive rate
     * @param c Number of bits used for counting
     */
    ...
    /**
     * Constructs an new Counting Bloom Filter. See constructor of superclass
     * for details. Using c=4 is the best choice for most applications. Couting
     * Bloom Filters behave like normal Bloom Filter but support a new
     * operation: {@link #remove(Object)}. Elements can thus be added and
     * removed from the bloom filter without loss of information.
     *
    ...
    /**
     * Set an overflow hand which is called every time a counter has an
     * overflow. This only happens if much more object are added to the couting
     * bloom filter than it was configured for.
     *
     * @param callback the overflow callback to be executed
     */
     ...
     *
     * @param values objects to be deleted
     */
    ...
    /**
     * Decrements the internal counter upon deletion and unsets the Bloom filter
     * bit if necessary.
     *
